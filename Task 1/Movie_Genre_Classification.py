...
genre_list = ['action','adult','adventure','animation','biography','comedycrime','documentry','family','fantacy','game-show','history','horror','music','musical','mystry','news','reality-tv','romance','sci-fi','short','sport','talk-show','thriller','war','western']
...
test_movies_names = test_data['MOVIE_NAME']
predicted_genres = mlb.inverse_transform(y_pred)
test_results = pd.DataFrame({'MOVIE_NAME': test_movies_names, 'PREDICTED_GERNES': predicted_genres })


test_results['PREDICTED_GENRES'] = test_results['PREDICTED_GERNES'].apply(lambda genres: [fallback_genre] if len(genres) == 0 else genres)
...
print("Model evaluation results and metrics have been saved in model_evaluation.txt")